...
define([
    ...
    return declare([GraphicsLayer], {
        ...
        mapIds: function(points) {
            // summary:
            //      gets the [[id,x,y]]
            // points
            console.log('app.ClusterFeatureLayer::mapIds', arguments);

            return arrayUtils.map(points, function(p){
                return p[0];
            });
        },
        mapPoints: function(points){
            return arrayUtils.map(points, function(p){
                return {x: p[1], y: p[2], id: p[0], attributes: {}};
            });
        },
        _onIdsReturned: function(results) {
            this._objectIdCache = this.mapIds(results.points);
            this._onFeaturesReturned({features: this.mapPoints(results.points)});
        ...
        _onFeaturesReturned: function(results) {
            ...
            var features;
            if (this.nativeGeometryType === 'esriGeometryPolygon') {
